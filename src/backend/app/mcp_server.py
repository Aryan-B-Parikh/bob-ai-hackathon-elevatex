...
def _full(db, persist=False): return pipeline.build_full(db, persist=persist)
...
def forecast_congestion(zone: str = "Z-PORT") -> dict:
    if zone not in ref.ALL_ZONES: return {"error": f"unknown zone {zone}", "valid_zones": ref.ALL_ZONES}
    db = _session()
    try:
        f = _full(db, persist=False)
        return {"zone": zone, "forecast": forecast_to_dict(f["forecasts"][zone]), "summary_all_zones": {z: {"current_index": fc.current["index"], "peak_index": fc.peak["index"], "peak_hour": fc.peak["hour"]} for z, fc in f["forecasts"].items()}}
    finally: db.close()
...
def rank_hotspots() -> dict:
    db = _session()
    try: return _full(db, persist=False)["hotspots"]
    finally: db.close()
...
def detect_anomalies() -> dict:
    db = _session()
    try:
        f = _full(db, persist=False); return {"anomalies": f["anomalies"], "method": "scikit-learn IsolationForest"}
    finally: db.close()
...
def optimise_berth_cranes(crane_factor: float = 1.0, move_rate_per_crane_hour: float = 28.0) -> dict:
    ...
    try:
        f = _full(db, persist=False)
        out = pipeline.run_optimiser(f["ctx"], f["forecasts"], {"crane_factor": max(0.5, min(1.0, crane_factor)), "move_rate_per_crane_hour": max(20.0, min(35.0, move_rate_per_crane_hour))})
        return {k: out[k] for k in ("solver", "status", "objective", "solve_ms", "params", "metrics", "baseline", "deltas", "weights", "assignments", "deferred")}
    finally: db.close()
...
def recommend_routing() -> dict:
    db = _session()
    try:
        recs = _full(db, persist=False)["routing"]; counts = {}
        for r in recs: counts[r["option"]] = counts.get(r["option"], 0) + 1
        return {"recommendations": recs, "counts": counts, "total_savings_usd": sum(r["est_savings_usd"] for r in recs)}
    finally: db.close()
...
def generate_operations_plan(include_text: bool = False) -> dict:
    db = _session()
    try:
        plan = _full(db, persist=False)["plan"]; out = {"summary": plan["summary"], "shifts": plan["shifts"]}
        if include_text: out["text"] = plan["text"]
        return out
    finally: db.close()
...
def simulate_scenario(crane_factor: float = 0.75, move_rate_per_crane_hour: float = 24.0) -> dict:
    ...
    try:
        f = _full(db, persist=False)
        base = pipeline.run_optimiser(f["ctx"], f["forecasts"], {}); scen = pipeline.run_optimiser(f["ctx"], f["forecasts"], {"crane_factor": max(0.5, min(1.0, crane_factor)), "move_rate_per_crane_hour": max(20.0, min(35.0, move_rate_per_crane_hour))})
        return {"params": scen["params"], "baseline": base["metrics"], "scenario": scen["metrics"], "impact": {"serviced": scen["metrics"]["serviced"] - base["metrics"]["serviced"], "moves": scen["metrics"]["total_moves"] - base["metrics"]["total_moves"], "avg_wait": round(scen["metrics"]["avg_wait_hours"] - base["metrics"]["avg_wait_hours"], 1), "makespan": round(scen["metrics"]["makespan_hours"] - base["metrics"]["makespan_hours"], 1)}}
    finally: db.close()
...
def query_vessels(status: str | None = None) -> dict:
    db = _session()
    try:
        f = _full(db, persist=False); assignment = {a["vessel_id"]: a for a in f["optimiser"]["assignments"]}; rows = []
        for v in f["ctx"].vessels:
            if status and v.status.upper() != status.upper(): continue
            d = vessel_to_dict(v); a = assignment.get(v.id); d["assignment"] = ({"berth_name": a["berth_name"], "terminal_code": a["terminal_code"], "start_hour": a["start_hour"], "cranes": a["cranes"]} if a else None); rows.append(d)
...
def get_terminals() -> dict:
    ...
    try:
        f = _full(db, persist=False)
        return {"source": "Port of Long Beach terminal fact sheets for capacity; live yard/gate state from current dataset", "terminals": [{"code": t.code, "name": t.name, "pier": t.pier, "berth_length_ft": t.berth_length_ft, "deepsea_berths": t.deepsea_berths, "gantry_cranes": t.gantry_cranes, "capacity_teu_m": t.capacity_teu_m, "zone_code": t.zone_code, "note": t.note, "available_cranes": f["ctx"].cranes.get(t.code), "yard_util_pct": f["ctx"].yard_util.get(t.code), "gate_queue": f["ctx"].gate_queue.get(t.code)} for t in f["ctx"].terminals]}
    finally: db.close()
...
def get_data_provenance() -> dict:
    db = _session()
    try:
        f = _full(db, persist=False); port = f["forecasts"]["Z-PORT"]
        return {"dataset_source": f["ctx"].dataset_source, "data_version": f["ctx"].data_version, "dataset_note": pipeline.DATASET_NOTE, "weather_used": bool(port.weather_used), "weather_source": "Open-Meteo" if port.weather_used else None, "model_version": port.model.get("model_version"), "confidence": port.confidence}
    finally: db.close()
...
def res_overview() -> str:
    db = _session()
    try: return json.dumps(pipeline.build_overview(*(_full(db, persist=False)[k] for k in ("ctx", "forecasts", "hotspots", "anomalies", "optimiser"))), default=str)
    finally: db.close()
...
def res_forecast(zone: str) -> str:
    ...
    try:
        f = _full(db, persist=False)
        return json.dumps(f["forecasts"][zone], default=str) if zone in f["forecasts"] else json.dumps({"error": f"unknown zone {zone}", "valid": ref.ALL_ZONES})
    finally: db.close()
...
def main(argv: list[str] | None = None) -> None:
    ...
    mcp.run(transport="stdio" if args.http is None else "streamable-http", **({} if args.http is None else {"host": "127.0.0.1", "port": args.http}))
