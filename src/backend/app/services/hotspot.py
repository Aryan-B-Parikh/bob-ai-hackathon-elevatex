...
# Gate pressure is normalised against the terminal's real hourly truck throughput
# (REAL POLB fact-sheet gate lanes/trucks-per-hour) so a random-walk queue cannot
# masquerade as the binding constraint (AUDIT B10).
...
def _pressure(ctx, zone_code: str, fpoint) -> dict[str, float]:
    ...
    # gate pressure: gate queue relative to one hour of real lane throughput
    gate_q = ctx.gate_queue.get(term.code, 0) if term else 0
    gate = min(1.0, gate_q / max(1.0, _GATE_TPH.get(term.code, 120.0) if term else 120.0))
...
def compute_hotspots(ctx, forecasts: dict, anomalies: list[dict]) -> dict:
    ...
    for zone_code, fc in forecasts.items():
        ...
        # data-support confidence, then blended with the forecast's own confidence
        data_conf = 0.9
        if len(recent) < 48:
            data_conf -= 0.25
        ...
        if any(v.unresolved for v in ctx.vessels if v.dest_zone_code == zone_code):
            data_conf -= 0.1
        data_conf = max(0.3, data_conf)
        fc_conf = float(getattr(fc, "confidence", 0.8) or 0.8)
        confidence = 0.5 * fc_conf + 0.5 * data_conf
        ...
        ranked.append({
            ...
            "binding_evidence": {
                "resource": binding,
                "pressure": round(binding_p, 3),
                "runner_up": runner_up,
                "runner_up_pressure": round(runner_up_p, 3),
                "margin": round(binding_p - runner_up_p, 3),
                "gate_throughput_ref_tph": _GATE_TPH.get(
                    next((t.code for t in ctx.terminals if t.zone_code == zone_code), ""), None),
            },
            ...
            "explanation": (
                ...
                f"(pressure {binding_p:.2f}, margin {binding_p - runner_up_p:+.2f} over {runner_up}). "
