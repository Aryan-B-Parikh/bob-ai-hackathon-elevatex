...
def _pressure(ctx, zone_code: str, fpoint) -> dict[str, float]:
    """Estimate resource utilisation from actual context state and forecast workload.

    Berth and crane pressure use the vessel work entering the zone by the forecast
    hour. Yard pressure uses forecast yard utilisation. Gate pressure uses the
    observed gate queue divided by the configured hourly gate throughput. These are
    capacity ratios, not arbitrary queue multipliers.
    """
    ...
    incoming = [v for v in ctx.vessels
                if v.dest_zone_code == zone_code and v.eta_hours <= fpoint.hour and not v.unresolved]
    # A three-crane service plan is the reference operating rate used by both the
    # simulator and optimiser for a normal vessel. The result is bounded to [0,1].
...
def compute_hotspots(ctx, forecasts: dict, anomalies: list[dict]) -> dict:
    ...
    for zone_code, fc in forecasts.items():
        ...
        ranked.append({
            ...
            "peak_index": peak.index, "risk_score": round(risk, 1),
            "binding_constraint": binding,
            "resource_pressure": {k: round(v, 3) for k, v in press.items()},
            "binding_evidence": {
                "resource": binding, "pressure": round(binding_p, 3),
                ...
                "incoming_vessels_by_peak": len([v for v in ctx.vessels if v.dest_zone_code == zone_code and v.eta_hours <= peak.hour and not v.unresolved]),
            },
            "components": {k: round(v, 3) for k, v in comp.items()},
            "weights": w, "confidence": round(max(0.05, min(0.98, confidence)), 2),
            "explanation": (
