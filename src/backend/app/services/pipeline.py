...
DATASET_NOTE = (
    ...
    "Operational history is labelled DEMO_AIS when generated locally; real NOAA AccessAIS "
    "imports are labelled AIS and flow through the same engines."
)
...
def run_forecasts(ctx: EngineContext, db: Session | None = None, force: bool = False) -> dict:
    ...
    # Include the full context fingerprint in the model-cache namespace via `source`.
...
def persist_forecast_run(db: Session, forecasts: dict, ctx: EngineContext) -> int:
    ...
    run = ForecastRun(model_version=port.model["model_version"], algorithm="LightGBM", horizon_hours=fc_svc.HORIZON,
    ...
    for z, fc in forecasts.items():
        for p in fc.points:
            db.add(ForecastPoint(run_id=run.id, zone_code=z, hour=p.hour, ts=p.ts, index=p.index, queue=p.queue, wait=p.wait, yard_util_pct=p.yard_util, lo=p.lo, hi=p.hi))
