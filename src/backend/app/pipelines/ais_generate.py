...
This module intentionally generates synthetic data for offline demonstrations.
It uses the same schema and transformation path as real AccessAIS data, but the
loaded observations are explicitly labelled ``DEMO_AIS`` and ``is_measured=False``.
"""
...
def _pick_anchorage(rng: random.Random) -> tuple[str, float, float]:
    ...
    for a in _ANCHORAGES:
        ...
        if r <= 0:
            ...
            return zone, lat_ctr + rng.uniform(-dlat * 0.8, dlat * 0.8), lon_ctr + rng.uniform(-dlon * 0.8, dlon * 0.8)
    a = _ANCHORAGES[-1]; return a[0], a[1], a[2]


def _generate_vessel_track(rng: random.Random, arrival_dt: datetime, dwell_hours: float, vessel_class: dict) -> list[dict]:
    records: list[dict] = []
    _zone_code, anch_lat, anch_lon = _pick_anchorage(rng)
    approach_lat, approach_lon = rng.choice(_APPROACH_WAYPOINTS)
    approach_lat += rng.uniform(-0.05, 0.05); approach_lon += rng.uniform(-0.05, 0.05)
    mmsi = str(rng.randint(300_000_000, 500_000_000)); imo = f"IMO{rng.randint(7_000_000, 9_999_999)}"
    ...
    loa = int(vessel_class["loa"] + rng.gauss(0, 20)); beam = vessel_class["beam"]; draft = round(vessel_class["draft"] + rng.gauss(0, 0.4), 1)

    def add(dt: datetime, lat: float, lon: float, sog: float, cog: float, status: int):
        records.append({"MMSI": mmsi, "BaseDateTime": _fmt_dt(dt), "LAT": round(lat, 5), "LON": round(lon, 5), "SOG": round(max(0.0, sog + rng.gauss(0, 0.1)), 1), "COG": round((cog + rng.gauss(0, 2)) % 360, 1), "Heading": round((cog + rng.gauss(0, 5)) % 360, 0), "VesselName": name, "IMO": imo, "CallSign": call, "VesselType": vessel_class["type"], "Status": status, "Length": loa, "Width": beam, "Draft": draft, "Cargo": 71})

    approach_hours = rng.uniform(3.0, 8.0); t = arrival_dt - timedelta(hours=approach_hours + dwell_hours)
    sog = rng.uniform(13.0, 16.0); interval_min = rng.uniform(2, 4); mid_lat, mid_lon = _interp(approach_lat, approach_lon, anch_lat, anch_lon, 0.6)
    phase1_steps = max(3, int(approach_hours * 60 / interval_min * 0.6))
    for i in range(phase1_steps):
        frac = i / max(1, phase1_steps - 1); lat, lon = _interp(approach_lat, approach_lon, mid_lat, mid_lon, frac)
        add(t, lat, lon, sog, _bearing(approach_lat, approach_lon, anch_lat, anch_lon), 0); t += timedelta(minutes=interval_min + rng.gauss(0, 0.5))

    sog = rng.uniform(3.0, 6.0); interval_min = rng.uniform(3, 5); phase2_steps = max(2, int(approach_hours * 60 / interval_min * 0.4))
    for i in range(phase2_steps):
        frac = i / max(1, phase2_steps - 1); lat, lon = _interp(mid_lat, mid_lon, anch_lat, anch_lon, frac)
        add(t, lat, lon, sog * (1 - frac * 0.7), _bearing(mid_lat, mid_lon, anch_lat, anch_lon), 3); t += timedelta(minutes=interval_min + rng.gauss(0, 0.5))

    t = arrival_dt; depart_dt = arrival_dt + timedelta(hours=dwell_hours)
    while t < depart_dt:
        add(t, anch_lat + rng.gauss(0, 0.001), anch_lon + rng.gauss(0, 0.001), max(0.0, rng.gauss(0.1, 0.08)), rng.uniform(0, 360), 1)
        t += timedelta(minutes=rng.uniform(3, 10))

    depart_dest_lat = approach_lat + rng.uniform(-0.1, 0.1); depart_dest_lon = approach_lon + rng.uniform(-0.1, 0.1)
    sog = rng.uniform(8.0, 14.0)
    for i in range(rng.randint(4, 8)):
        frac = (i + 1) / 8; lat, lon = _interp(anch_lat, anch_lon, depart_dest_lat, depart_dest_lon, frac)
        add(t, lat, lon, sog, _bearing(anch_lat, anch_lon, depart_dest_lat, depart_dest_lon), 0); t += timedelta(minutes=rng.uniform(2, 5))
    return records


def generate_ais_csv(days: int = 14, seed: int = 20240817, arrivals_per_day: float = 8.0) -> str:
    rng = random.Random(seed); now = datetime.now(UTC).replace(minute=0, second=0, microsecond=0); start_dt = now - timedelta(days=days)
    all_records: list[dict] = []; t = start_dt; mean_gap_hours = 24.0 / arrivals_per_day
    while t < now:
    ...
    for k in range(3):
        bunch_t = now - timedelta(hours=144 - k * 4); vc_ulcv = next(c for c in _CLASSES if c["cls"] == "ULCV")
        all_records.extend(_generate_vessel_track(rng, bunch_t, rng.uniform(24, 48), vc_ulcv))
    for anchor_entry in _ANCHORAGES:
        zone_code, lat_ctr, lon_ctr, dlat, dlon, _ = anchor_entry
        arrive_t = now - timedelta(hours=rng.uniform(6, 18)); all_records.extend(_generate_vessel_track(rng, arrive_t, rng.uniform(36, 96), _pick_class(rng)))
    all_records.sort(key=lambda r: r["BaseDateTime"])
    buf = StringIO(); writer = csv.DictWriter(buf, fieldnames=_COLUMNS); writer.writeheader(); writer.writerows(all_records); return buf.getvalue()


def generate_and_load(days: int = 14, seed: int = 20240817) -> dict:
    """Generate synthetic AccessAIS-shaped data and load it as DEMO_AIS."""
    import tempfile
    ...
    with tempfile.NamedTemporaryFile(mode="w", suffix=".csv", delete=False, encoding="utf-8") as raw_f:
        raw_f.write(csv_content); raw_path = raw_f.name
    ...
    try:
        ...
        import_stats = ais_pipeline.import_series(series_path, source="DEMO_AIS", is_measured=False)
    finally:
...
def main(argv: list[str] | None = None) -> None:
    p = argparse.ArgumentParser(prog="app.pipelines.ais_generate", description="Generate synthetic San Pedro Bay AccessAIS-shaped data")
    p.add_argument("--days", type=int, default=14); p.add_argument("--seed", type=int, default=20240817); p.add_argument("--out", type=str, default=""); p.add_argument("--no-load", action="store_true")
