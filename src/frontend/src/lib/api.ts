...
export const api = {
  overview: async () => { const raw = await get("/api/overview"); const parsed = OverviewResponseSchema.safeParse(raw); if (!parsed.success) console.warn("Overview payload schema warning:", parsed.error); return raw; },
  forecast: (zone: string) => get(`/api/forecast?zone=${encodeURIComponent(zone)}`), terminals: () => get("/api/terminals"), vessels: () => get("/api/vessels"), hotspots: () => get("/api/hotspots"), routing: () => get("/api/routing"), plan: () => get("/api/plan"), optimiseLatest: () => get("/api/optimise/latest"),
  optimise: (body: { crane_factor: number; move_rate_per_crane_hour: number; incremental?: boolean; tidal?: boolean }) => post("/api/optimise", body),
  bobHistory: () => get("/api/bob"), bobStatus: () => get("/api/bob/status"), bob: (message: string) => post("/api/bob", { message }), exportUrl: (type: string) => `/api/export?type=${type}`,
  health: () => get<{ status: string }>("/health"), anomalies: () => get("/api/anomalies"), quality: () => get<QualityResponse>("/api/quality"), weather: (hours = 72) => get<WeatherResponse>(`/api/weather?hours=${hours}`), uploadSchedule: (file: File) => upload<UploadResponse>("/api/vessels/upload", file),
  scenario: (body: { crane_factor: number; move_rate_per_crane_hour: number }) => post("/api/scenarios", body), scenarioExtended: (body: ScenarioExtendedRequest) => post<ScenarioExtendedResponse>("/api/scenarios/extended", body), scenarioRollback: (id: number) => post<{ restored: boolean }>(`/api/scenarios/${id}/rollback`),
};
