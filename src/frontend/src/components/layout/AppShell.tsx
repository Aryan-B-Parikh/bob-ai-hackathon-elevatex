...
export function AppShell({ activeTab, onSelectTab, activeTitle, activeSubtitle, onRefresh, refreshing = false, children }: AppShellProps) {
  ...
  return (
    <div className="flex h-screen w-screen overflow-hidden bg-[var(--bg-app)] text-[var(--text-primary)]">
      ...
      {mobileMenuOpen && (
        <div role="dialog" aria-modal="true" aria-label="Mobile Navigation Menu" className="fixed inset-0 z-50 md:hidden flex">
          <div className="fixed inset-0 bg-black/65 backdrop-blur-xs" onClick={() => setMobileMenuOpen(false)} aria-hidden="true" />
          <div className="relative z-50 w-72 max-w-[85vw] h-full flex flex-col shadow-2xl animate-in slide-in-from-left duration-200">
            <Sidebar activeTab={activeTab} onSelectTab={(tab) => { onSelectTab(tab); setMobileMenuOpen(false); }} collapsed={false} onCloseMobile={() => setMobileMenuOpen(false)} className="w-full h-full border-r-0" />
          </div>
        </div>
      )}
          ...
          <footer className="border-t border-[var(--border-subtle)] bg-[var(--bg-surface)] px-6 py-4 text-[11px] text-[var(--text-muted)] space-y-1 select-none shrink-0">
            <div className="flex flex-wrap items-center justify-between gap-2">
              <div className="flex flex-wrap items-center gap-x-3 gap-y-1">
                <span><strong className="text-[var(--text-secondary)] font-medium">Engines:</strong> LightGBM Quantile Regression (24/48/72h) · OR-Tools CP-SAT (BAP/QCAP) · SimPy Operations Layer</span>
                <span>·</span>
                <span><strong className="text-[var(--text-secondary)] font-medium">Anomaly:</strong> Isolation Forest</span>
                <span>·</span>
                <span><strong className="text-[var(--text-secondary)] font-medium">Agent:</strong> IBM Bob via MCP</span>
              </div>
              <div className="font-mono text-[10px] text-[var(--text-muted)]">PortPulse AI v0.1.0 · San Pedro Bay (POLB/POLA)</div>
            </div>
            <div className="text-[10px] text-[var(--text-muted)]">Data Honesty: Terminal capacity = official Port of Long Beach fact sheets. Vessel queue and 14-day hourly history = labelled synthetic <code className="text-[var(--text-accent)] font-mono">DEMO_AIS</code>.</div>
          </footer>
        </main>
      </div>
    </div>
