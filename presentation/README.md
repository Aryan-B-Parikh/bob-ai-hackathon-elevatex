...
2. **Data** — REAL: Port of Long Beach terminal fact sheets (LBCT Pier E 4,200 ft / 3 deepsea berths / 18 STS cranes, 3.5M+ TEU; ITS Pier G 4,250 ft / 14; PCT Pier J 5,902 ft / 14; TTI Pier T 5,000 ft / 16; port-wide 80 berths / 10 piers / 71 post-Panamax cranes). REAL pipeline, labelled demo default: NOAA AccessAIS series via `scripts/ais` (DEMO_AIS disclosure + swap path).
...
9. **Demo** — 3–5 min real run: cold start per `docs/setup-guide.md` → Overview → Forecast → Run optimiser → Routing → Regenerate plan → ask Bob; honest limitations slide (demo dataset labelled, heuristic optimiser, 13-berth subset).
